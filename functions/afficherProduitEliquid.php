...
$sql = "SELECT * FROM Products WHERE Categorie= 1";
...
if(isset($_POST['search'])){
    $search = "%$_POST[search]%";
    $sql = "SELECT * FROM Products WHERE Name LIKE :search AND Categorie = 1";
    $query = $dbh->prepare($sql);
...
if(isset($_POST['trierPar'])){
    if($_POST['trierPar']=='mostPopural'){
        $sql = "SELECT * FROM Products WHERE Categorie= 1 ORDER BY Popular DESC";
        $query = $dbh->prepare($sql);
        $query->execute();
    ...
    elseif($_POST['trierPar']=='lessPopular'){
        $sql = "SELECT * FROM Products WHERE Categorie= 1 ORDER BY Popular ASC";
        $query = $dbh->prepare($sql);
        $query->execute();
    ...
    elseif($_POST['trierPar']=='decreasing'){
        $sql = "SELECT * FROM Products WHERE Categorie= 1 ORDER BY Price DESC";
        $query = $dbh->prepare($sql);
        $query->execute();
    ...
    elseif($_POST['trierPar']=='increasing'){
        $sql = "SELECT * FROM Products WHERE Categorie= 1 ORDER BY Price ASC";
        $query = $dbh->prepare($sql);
        $query->execute();
